...
const getSelectionArea = e => {
  ...
  if (input.__IS_SHOWING_MARKER) {
    ...
    input.__MARKER.setAttribute(
      "style",
      `left: ${newLeft - scrollLeft}px; top: ${startTop - scrollTop}px;`
    );
  }
};
...
const showCustomUI = e => {
  ...
  const { paddingRight, lineHeight } = getComputedStyle(input);
  // create a function that will handle clicking off of the input and hide the marker
  const processClick = evt => {
    if (e !== evt && evt.target !== e.target) {
      toggleCustomUI();
  ...
  const filterList = () => {
    const filter = value
      .slice(input.__EDIT_START + 1, selectionStart)
      .toLowerCase();
    ...
    if (!filteredSuggestions.length)
      filteredSuggestions.push("No suggestions available...");
    ...
    if (input.__CUSTOM_UI.firstChild)
      input.__CUSTOM_UI.replaceChild(
        suggestedList,
        input.__CUSTOM_UI.firstChild
      );
  ...
  const selectItem = (selected, click = false) => {
    ...
    const end = input.value.slice(
      click ? selectionStart + 1 : selectionStart,
      input.value.length
    );
  ...
  const toggleCustomUI = () => {
    ...
    if (input.__IS_SHOWING_CUSTOM_UI) {
      ...
      input.__CUSTOM_UI.setAttribute(
        "style",
        `left: ${newLeft}px; top: ${newTop}px;`
      );
    }
  ...
  if (which === 35 && previousChar === "") {
    ...
    switch (which) {
      ...
      case 32:
        ...
        break;
      case 8:
        if (selectionStart === input.__EDIT_START) toggleCustomUI();
        else filterList();
        break;
      case 13:
        if (input.__SELECTED_ITEM) {
          e.preventDefault();
          selectItem(
            input.__CUSTOM_UI.querySelector(".custom-suggestions--active")
              .textContent
          );
          toggleCustomUI();
        } else {
