...
function reorderArray(event, originalArray) {
  ...
  var reorderedItems = [].concat(reorderArray(remainingItems.slice(0, event.newIndex)), [movedItem], reorderArray(remainingItems.slice(event.newIndex)));
...
function checkDrag(event) {
  return [].concat(reorderArray(event.dataTransfer.types)).includes("application/x-moz-file");
}
...
function dropFileHandler(ev) {
  ...
  console.log("File(s) dropped"); // Prevent default behavior (Prevent file from being opened)
    ...
    for (var i = 0; i < ev.dataTransfer.items.length; i++) {
      ...
      if (ev.dataTransfer.items[i].kind === "file") {
        ...
        reader.onload = function (event) {
          ...
          var lines = [];
          lines = event.target.result.replace(/\r\n/g, "\r").replace(/\n/g, "\r").split(/\r/);
